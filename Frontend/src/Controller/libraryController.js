...
export const createGame = async (gameData) => {
  try {
    ...
    const response = await fetch("http://192.168.1.13:8000/libreria/", {
      method: "POST",
      body: formData, 
    });
...
export const getGameById = async (id) => {
  const response = await fetch(`http://192.168.1.13:8000/libreria/${id}/`);
...
export async function setFechaTerminado({ id, fecha }) {
  const res = await fetch(`http://192.168.1.13:8000/libreria/${id}/`, {
    method: "PATCH",
    headers: {
